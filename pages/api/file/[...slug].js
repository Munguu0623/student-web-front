...
export default async function handler(req, res) {
  ...
  if (
    ...
    const filePath = process.env.NEXT_PUBLIC_UPLOAD_PATH + "/" + req.query.file;
    ...
    const videoPath =
      process.env.NEXT_PUBLIC_UPLOAD_PATH + "/" + req.query.file;
    console.log("videoPath --------->", videoPath);

    fs.readFile(videoPath, function (err, content) {
      if (err) {
        fs.readFile(
          process.env.NEXT_PUBLIC_UPLOAD_PATH + "/no_image.png",
          (err, video) => {
            if (err) {
              res.end("no video");
              console.log(err);
